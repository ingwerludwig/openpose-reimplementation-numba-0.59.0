...
post_dir = os.path.join(current_dir, '..')
absolute_post_dir = os.path.abspath(post_dir)
sys.path.append(absolute_post_dir)

import visualizations as vis
import configs.draw_config as draw_config

model_path = os.path.join(absolute_post_dir, 'trained_models/model11_test-15Sun1219-2101')

class CamApp:
    def __init__(self):
        self.model_wrapper = ModelWrapper(model_path)

        self.cam = cv2.VideoCapture(1)  # 0 -> index of camera
